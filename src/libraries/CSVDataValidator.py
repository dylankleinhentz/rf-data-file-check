...
class CSVDataValidator:
    """Main validation library for CSV data files"""
    ...
    def validate_unique_values(self, column_name: str) -> bool:
        ...
        if duplicates:
            dup_str = ', '.join([f"'{k}' (rows {v})" for k, v in duplicates.items()])
            self.errors.append(
        ...
        for idx, row in enumerate(self.data, 1):
            ...
            try:
                ...
                if min_value is not None and num_value < min_value:
                    ...
                    out_of_range.append((idx, value, f"greater than {max_value}"))
            except ValueError:
                pass
        
        if out_of_range:
            err_str = ', '.join([f"row {r[0]}='{r[1]}' ({r[2]})" for r in out_of_range])
            self.errors.append(
